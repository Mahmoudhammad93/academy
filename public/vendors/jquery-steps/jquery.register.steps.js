...
!(function (a, b) {
    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
                ...
                s = '<li><a href="#{0}" role="menuitem">{1}</a></li>',
                f = "";
            b.forceMoveForward || (f += e.format("previous", b.labels.previous)),
                (f += e.format("next", b.labels.next)),
                b.enableFinishButton && (f += s.format("finish", b.labels.finish)),
    ...
    var $ = (a.fn.steps.contentMode = { html: 0, iframe: 1, async: 2 }),
        ...
        cb = (a.fn.steps.defaults = {
            ...
            onInit: function () {},
            labels: { cancel: "Cancel", current: "current step:", pagination: "Pagination", finish: "Submit", next: "Next", previous: "Previous", loading: "Loading ..." },
        });
})(jQuery);
